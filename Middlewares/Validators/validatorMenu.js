...
module.exports = (req, res, next) => {
  try {
    if (req.body.idRestaurant == req.params.idRestaurant) {
      if ((req.body.name.length >= 2 && req.body.name.length < 30 && _.isString(req.body.name)) && (req.body.description.length >= 2 && req.body.description.length < 200 && _.isString(req.body.description))) {
        Menus.findOne({ name: req.body.name, idRestaurant: req.params.idRestaurant })
          .then(menu => {
